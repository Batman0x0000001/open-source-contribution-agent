...
8. Call `submit_delivery_draft` with only the draft content requested by its schema. The trusted Worker binds Job and workspace identity. Once accepted, stop immediately.

## Success evidence and checkpoint

The transcript must contain a successful primary test after the final edit, later independent verification evidence, a still later complete Git snapshot, and finally the delivery draft. Do not run another test after Verify `PASS`, do not Verify after the final Git snapshot, and do not call tools after the delivery draft succeeds. If primary tests cannot run, request the existing test waiver. If Verify is `PARTIAL`, use only the independent-verification waiver; `FAIL` cannot be waived. Do not weaken tests, silently broaden scope, commit, push, discard changes, or claim success after failing validation.
